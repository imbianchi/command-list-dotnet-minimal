...
builder.Services.AddDbContext<AppDbContext>(opt => opt.UseSqlServer(sqlConBuilder
    .ConnectionString));
...
app.MapGet("api/vi/commands/{id}", async (ICommandRepo repo, IMapper mapper, int id) =>
{
...
app.MapPost("api/v1/commands", async (ICommandRepo repo, IMapper mapper, CommandCreateDto commandCreateDto) =>
{
    ...
    return Results.Created($"api/v'/commmands/{commandReadDto.Id}", commandReadDto);
});
...
app.MapDelete("api/v1/commands", async (ICommandRepo repo, IMapper mapper, int id) =>
{
    var command = await repo.GetCommandById(id);
